...
describe GildedRose do
  ...
  describe "#update_quality" do
    ...
    context "for any general item" do
      ...
      it "degrades the sell_in and quality (-2) when sell_in < 0" do
      ...
      it "degrades the sell_in and quality (-1) when sell_in < 0" do
      ...
      it "degrades only the sell_in when sell_in < 0 and quality = 0" do
        items = [Item.new("foo", 0, 0)]
        GildedRose.new(items).update_quality()
        expect(items[0].sell_in).to eq -1
        expect(items[0].quality).to eq 0
